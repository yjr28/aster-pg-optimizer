...
@pytest.mark.parametrize(
    ...
    (
        ("relations", "relkind", ""),
        ("relations", "estimated_rows", "100"),
        ("relations", "pages", True),
        ("relations", "pages", -1),
        ("indexes", "index_definition", 123),
        ("statistics_state", "n_live_tup", -1),
        ("statistics_state", "last_analyze", 123),
        ("statistics_state", "analyze_count", False),
        ("statistics_targets", "statistics_target", "-1"),
    ),
)
def test_environment_diff_rejects_catalog_rows_with_impossible_value_types(
    section, field, value
):
    before = _environment()
    after = deepcopy(before)
    after["postgres"][section][0][field] = value
    _refresh_hashes(after)

    with pytest.raises(ValueError, match=rf"{section} row field {field} has invalid"):
