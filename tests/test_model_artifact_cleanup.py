...
def test_save_model_retains_metadata_evidence_when_metadata_backup_cleanup_fails(
    ...
    def fail_first_metadata_backup_cleanup(self, *args, **kwargs):
        nonlocal failed_cleanup
        if ".metadata.json.backup." in self.name and not failed_cleanup:
            failed_cleanup = True
            raise OSError("simulated metadata backup cleanup failure")
        return original_unlink(self, *args, **kwargs)

    monkeypatch.setattr(model_io.joblib, "dump", write_new_model)
    monkeypatch.setattr(Path, "unlink", fail_first_metadata_backup_cleanup)
